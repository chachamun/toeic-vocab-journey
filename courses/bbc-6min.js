/* BBC 6 Minute English 影片課程 — 由 tools/build_video_units.py 產生，請勿手改。
   要改內容：編 tools/episodes/<影片ID>.json 後重跑 `python tools/build_video_units.py course`。
   逐字稿（含原文出處）在 transcripts/<影片ID>.json，App 看影片時才載入。 */
TVJ.register({
 "id": "bbc-6min",
 "name": "BBC 6 Minute English",
 "kind": "video",
 "units": [
  {
   "id": "yt-rCIs5x0SGTY",
   "label": "BBC 001",
   "theme": "面對拒絕",
   "themeEn": "How do you feel when someone says 'no'?",
   "video": "https://youtu.be/rCIs5x0SGTY",
   "source": "https://www.bbc.co.uk/learningenglish/english/features/6-minute-english_2026/ep-260917",
   "publisher": "BBC Learning English《6 Minute English》",
   "release": "2026-09-23",
   "transcript": "transcripts/rCIs5x0SGTY.json",
   "lines": 84,
   "audioSrc": "",
   "words": [
    {
     "n": 1,
     "w": "rejection",
     "s": 3,
     "ph": "[rɪˋdʒɛkʃən]",
     "pos": [
      {
       "p": "n.",
       "m": "拒絕；被拒絕"
      }
     ],
     "ex": [
      {
       "en": "Well, nobody likes rejection.",
       "zh": "嗯，沒有人喜歡被拒絕。",
       "t": 28.2
      }
     ],
     "fam": [
      "reject v. 拒絕，駁回"
     ],
     "tips": [
      {
       "k": "常考語句",
       "t": "reject an offer / a proposal / an application 拒絕提議／提案／申請；a letter of rejection 拒絕信（應徵落選通知）。多益常考動詞 reject 與名詞 rejection 的詞性區分。"
      }
     ]
    },
    {
     "n": 2,
     "w": "spike",
     "s": 2,
     "ph": "[spaɪk]",
     "pos": [
      {
       "p": "v.",
       "m": "急遽上升，突然增加"
      },
      {
       "p": "n.",
       "m": "急遽上升"
      }
     ],
     "ex": [
      {
       "en": "If something spikes, it increases sharply.",
       "zh": "如果某樣東西 spike，就是它急遽上升。",
       "t": 188.4
      }
     ],
     "tips": [
      {
       "k": "常考語句",
       "t": "a spike in sales / demand / prices 銷售／需求／價格暴增。多益圖表題常用 spike、surge、soar 描述數字急升。"
      }
     ]
    },
    {
     "n": 3,
     "w": "socially anxious",
     "s": 1,
     "ph": "[ˋsoʃəlɪ ˋæŋkʃəs]",
     "pos": [
      {
       "p": "adj.",
       "m": "有社交焦慮的"
      }
     ],
     "ex": [
      {
       "en": "Someone who is socially anxious is nervous or worried about speaking to other people.",
       "zh": "有社交焦慮的人，會對和別人說話感到緊張或擔心。",
       "t": 195.0
      }
     ],
     "fam": [
      "anxiety n. 焦慮",
      "anxious adj. 焦慮的；渴望的"
     ],
     "tips": [
      {
       "k": "易混淆",
       "t": "be anxious about 擔心～；be anxious to do 渴望做～（多益常考後者：We are anxious to hear from you. 我們急於收到您的回覆）。"
      }
     ]
    },
    {
     "n": 4,
     "w": "come easier",
     "s": 1,
     "ph": "",
     "pos": [
      {
       "p": "phr.",
       "m": "（做起來）變得比較容易"
      }
     ],
     "ex": [
      {
       "en": "And once I'd done one, the second one came easier and the third one came even easier.",
       "zh": "做完一次之後，第二次就容易多了，第三次又更容易。",
       "t": 173.8
      }
     ],
     "tips": [
      {
       "k": "文法",
       "t": "come + 形容詞比較級，表示「變得更～」，屬口語說法。正式書寫可改用 become easier。"
      }
     ]
    },
    {
     "n": 5,
     "w": "perfectionism",
     "s": 1,
     "ph": "[pɚˋfɛkʃən͵ɪzəm]",
     "pos": [
      {
       "p": "n.",
       "m": "完美主義"
      }
     ],
     "ex": [
      {
       "en": "Perfectionism is the feeling that everything you do needs to be perfect.",
       "zh": "完美主義就是覺得自己做的每件事都必須完美。",
       "t": 275.6
      }
     ],
     "fam": [
      "perfectionist n. 完美主義者"
     ]
    },
    {
     "n": 6,
     "w": "appraise",
     "s": 3,
     "ph": "[əˋprez]",
     "pos": [
      {
       "p": "v.",
       "m": "評價，評估"
      }
     ],
     "ex": [
      {
       "en": "Young people are also worried about how people will appraise them, which means judge them.",
       "zh": "年輕人也擔心別人會怎麼評價他們。",
       "t": 280.5
      }
     ],
     "fam": [
      "appraisal n. 評價，評估"
     ],
     "tips": [
      {
       "k": "常考語句",
       "t": "performance appraisal 績效考核（多益高頻）；appraise the value of a property 評估房產價值。"
      }
     ]
    },
    {
     "n": 7,
     "w": "boost",
     "s": 3,
     "ph": "[bust]",
     "pos": [
      {
       "p": "v.",
       "m": "提升，增強"
      },
      {
       "p": "n.",
       "m": "提升，推動"
      }
     ],
     "ex": [
      {
       "en": "But people who post the videos say they boost their confidence.",
       "zh": "但發布影片的人說，這能提升他們的自信。",
       "t": 228.0
      }
     ],
     "tips": [
      {
       "k": "常考語句",
       "t": "boost sales / productivity / morale 提高銷售／生產力／士氣，多益商業情境高頻。"
      }
     ]
    }
   ],
   "cloze": [
    {
     "s": "Well, nobody likes ___.",
     "a": "rejection",
     "opts": [
      "rejection",
      "perfectionism",
      "adrenaline",
      "therapy"
     ]
    },
    {
     "s": "If something ___, it increases sharply.",
     "a": "spikes",
     "opts": [
      "spikes",
      "appraises",
      "rejects",
      "boosts"
     ]
    },
    {
     "s": "Someone who is socially ___ is nervous or worried about speaking to other people.",
     "a": "anxious",
     "opts": [
      "anxious",
      "confident",
      "perfect",
      "friendly"
     ]
    },
    {
     "s": "And once I'd done one, the second one came ___.",
     "a": "easier",
     "opts": [
      "easier",
      "harder",
      "faster",
      "later"
     ]
    },
    {
     "s": "Claudia says there's an increase in ___ among young people.",
     "a": "perfectionism",
     "opts": [
      "perfectionism",
      "rejection",
      "confidence",
      "therapy"
     ]
    },
    {
     "s": "Young people are also worried about how people will ___ them, which means judge them.",
     "a": "appraise",
     "opts": [
      "appraise",
      "reject",
      "spike",
      "boost"
     ]
    },
    {
     "s": "But people who post the videos say they ___ their confidence.",
     "a": "boost",
     "opts": [
      "boost",
      "appraise",
      "reject",
      "spike"
     ]
    },
    {
     "s": "It's a social media ___ where people record themselves talking to strangers and sometimes being rejected.",
     "a": "trend",
     "opts": [
      "trend",
      "therapy",
      "rejection",
      "habit"
     ]
    }
   ],
   "comp": [
    {
     "q": "節目問答題：以下哪件事「沒有」發生在華特・迪士尼成功之前？",
     "a": "他的第一部長片失敗了",
     "opts": [
      "他的第一部長片失敗了",
      "他丟了畫家的工作",
      "他的第一個事業資金用光了"
     ]
    },
    {
     "q": "「拒絕療法（rejection therapy）」是什麼？",
     "a": "拍下自己向陌生人提出請求、有時被拒絕的社群潮流",
     "opts": [
      "拍下自己向陌生人提出請求、有時被拒絕的社群潮流",
      "經過驗證的正式心理治療方法",
      "教人如何拒絕別人的課程",
      "一種線上面試訓練"
     ]
    },
    {
     "q": "Hannah 實際嘗試後發現什麼？",
     "a": "大多數陌生人都很友善、樂於幫忙",
     "opts": [
      "大多數陌生人都很友善、樂於幫忙",
      "幾乎每個人都拒絕了她",
      "她的社交焦慮變得更嚴重",
      "計程車司機讓她開了車"
     ]
    },
    {
     "q": "心理學家 Claudia 認為，年輕人可能想主動找拒絕的原因是？",
     "a": "完美主義增加，擔心別人的評價",
     "opts": [
      "完美主義增加，擔心別人的評價",
      "想在社群媒體上賺錢",
      "心理學家建議他們這樣做",
      "他們不在乎別人的看法"
     ]
    }
   ]
  },
  {
   "id": "yt-xwseWCSXD3Y",
   "label": "BBC 002",
   "theme": "描述氣味",
   "themeEn": "How are smell adjectives different?",
   "video": "https://youtu.be/xwseWCSXD3Y",
   "source": "https://www.bbc.co.uk/learningenglish/english/features/6-minute-english_2026/ep-260827",
   "publisher": "BBC Learning English《6 Minute English》",
   "release": "2026-09-23",
   "transcript": "transcripts/xwseWCSXD3Y.json",
   "lines": 75,
   "audioSrc": "",
   "words": [
    {
     "n": 1,
     "w": "stinky",
     "s": 1,
     "ph": "[ˋstɪŋkɪ]",
     "pos": [
      {
       "p": "adj.",
       "m": "很臭的，難聞的"
      }
     ],
     "ex": [
      {
       "en": "I might think a perfume is stinky, an adjective meaning bad smelling, while Neil might find the same perfume fragrant, meaning pleasant smelling.",
       "zh": "我可能覺得某款香水很臭，而 Neil 可能覺得同一款香水很香。",
       "t": 174.0
      }
     ],
     "fam": [
      "stink v. 發出臭味"
     ],
     "ant": "fragrant 香的"
    },
    {
     "n": 2,
     "w": "fragrant",
     "s": 2,
     "ph": "[ˋfreɡrənt]",
     "pos": [
      {
       "p": "adj.",
       "m": "香的，芬芳的"
      }
     ],
     "ex": [
      {
       "en": "I could say, oh this perfume you think is really fragrant, I think is stinky, and both can be true.",
       "zh": "你覺得很香的這瓶香水，我覺得很臭，而兩者都可以成立。",
       "t": 163.1
      }
     ],
     "fam": [
      "fragrance n. 香味；香水"
     ],
     "tips": [
      {
       "k": "常考語句",
       "t": "fragrance-free products 無香料產品；a line of fragrances 一系列香水。名詞 fragrance 在多益產品說明中常見。"
      }
     ]
    },
    {
     "n": 3,
     "w": "subjective",
     "s": 2,
     "ph": "[səbˋdʒɛktɪv]",
     "pos": [
      {
       "p": "adj.",
       "m": "主觀的"
      }
     ],
     "ex": [
      {
       "en": "It seems to be a subjective thing.",
       "zh": "它似乎是一種主觀的東西。",
       "t": 161.0
      }
     ],
     "ant": "objective 客觀的",
     "tips": [
      {
       "k": "易混淆",
       "t": "subjective 主觀的 ↔ objective 客觀的。objective 當名詞是「目標」（the main objective of the meeting 會議的主要目標），多益更常考這個用法。"
      }
     ]
    },
    {
     "n": 4,
     "w": "visual",
     "s": 2,
     "ph": "[ˋvɪʒʊəl]",
     "pos": [
      {
       "p": "adj.",
       "m": "視覺的"
      }
     ],
     "ex": [
      {
       "en": "The adjective 'visual' means related to seeing, and 'auditory' means related to hearing.",
       "zh": "形容詞 visual 的意思是和視覺有關的，auditory 則是和聽覺有關的。",
       "t": 254.0
      }
     ],
     "tips": [
      {
       "k": "常考語句",
       "t": "visual aids 視覺輔助工具（簡報用的圖表、投影片），多益簡報情境常見。"
      }
     ]
    },
    {
     "n": 5,
     "w": "auditory",
     "s": 1,
     "ph": "[ˋɔdə͵torɪ]",
     "pos": [
      {
       "p": "adj.",
       "m": "聽覺的"
      }
     ],
     "ex": [
      {
       "en": "Asifa's research shows that, here in the UK at least, people talk about visual or auditory things much more than they talk about smells.",
       "zh": "Asifa 的研究顯示，至少在英國，人們談論視覺或聽覺事物遠多於談論氣味。",
       "t": 246.0
      }
     ],
     "fam": [
      "audio n. 音訊",
      "audience n. 聽眾，觀眾",
      "auditorium n. 禮堂"
     ]
    },
    {
     "n": 6,
     "w": "olfactory",
     "s": 1,
     "ph": "[ɑlˋfæktərɪ]",
     "pos": [
      {
       "p": "adj.",
       "m": "嗅覺的"
      }
     ],
     "ex": [
      {
       "en": "The adjective for things related to smelling is 'olfactory'.",
       "zh": "和嗅覺有關的形容詞是 olfactory。",
       "t": 259.8
      }
     ]
    },
    {
     "n": 7,
     "w": "shut something down",
     "s": 3,
     "ph": "",
     "pos": [
      {
       "p": "phr. v.",
       "m": "使停止；關閉"
      }
     ],
     "ex": [
      {
       "en": "When children start talking about smells, especially unpleasant ones, parents often react by shutting the conversation down.",
       "zh": "當孩子開始談論氣味，尤其是難聞的氣味，父母常常會直接打斷這個話題。",
       "t": 267.1
      }
     ],
     "tips": [
      {
       "k": "常考語句",
       "t": "shut down a factory / a production line / the system 關閉工廠／生產線／系統。多益公告題常見：The plant will be shut down for maintenance. 工廠將停工維修。"
      }
     ]
    },
    {
     "n": 8,
     "w": "extensively",
     "s": 2,
     "ph": "[ɪkˋstɛnsɪvlɪ]",
     "pos": [
      {
       "p": "adv.",
       "m": "廣泛地，大量地"
      }
     ],
     "ex": [
      {
       "en": "Oxford University psychologist Asifa Majid has researched the subject of smell extensively.",
       "zh": "牛津大學心理學家 Asifa Majid 對嗅覺做過大量研究。",
       "t": 137.2
      }
     ],
     "fam": [
      "extensive adj. 廣泛的",
      "extend v. 延長"
     ],
     "tips": [
      {
       "k": "常考語句",
       "t": "travel extensively 到處出差；extensive experience in 在～方面有豐富經驗（履歷、徵才廣告高頻）。"
      }
     ]
    }
   ],
   "cloze": [
    {
     "s": "I might think a perfume is ___, an adjective meaning bad smelling.",
     "a": "stinky",
     "opts": [
      "stinky",
      "fragrant",
      "visual",
      "musty"
     ]
    },
    {
     "s": "Neil might find the same perfume ___, meaning pleasant smelling.",
     "a": "fragrant",
     "opts": [
      "fragrant",
      "stinky",
      "auditory",
      "subjective"
     ]
    },
    {
     "s": "It seems to be a ___ thing.",
     "a": "subjective",
     "opts": [
      "subjective",
      "objective",
      "visual",
      "olfactory"
     ]
    },
    {
     "s": "The adjective '___' means related to seeing.",
     "a": "visual",
     "opts": [
      "visual",
      "auditory",
      "olfactory",
      "fragrant"
     ]
    },
    {
     "s": "'Auditory' means related to ___.",
     "a": "hearing",
     "opts": [
      "hearing",
      "seeing",
      "smelling",
      "tasting"
     ]
    },
    {
     "s": "The adjective for things related to smelling is '___'.",
     "a": "olfactory",
     "opts": [
      "olfactory",
      "auditory",
      "visual",
      "subjective"
     ]
    },
    {
     "s": "When children start talking about smells, parents often react by shutting the conversation ___.",
     "a": "down",
     "opts": [
      "down",
      "up",
      "off",
      "out"
     ]
    },
    {
     "s": "Oxford University psychologist Asifa Majid has researched the subject of smell ___.",
     "a": "extensively",
     "opts": [
      "extensively",
      "rarely",
      "briefly",
      "recently"
     ]
    }
   ],
   "comp": [
    {
     "q": "節目問答題：哪個字是指「舊書的味道」？",
     "a": "vellichor",
     "opts": [
      "vellichor",
      "petrichor",
      "nidor"
     ]
    },
    {
     "q": "petrichor 是指什麼氣味？",
     "a": "雨後地面／泥土的味道",
     "opts": [
      "雨後地面／泥土的味道",
      "舊書的味道",
      "烹煮肉類與油脂的味道",
      "剛打開的咖啡粉香味"
     ]
    },
    {
     "q": "根據 Asifa Majid 的說法，為什麼 stinky、fragrant 這類字沒有說出氣味本身？",
     "a": "氣味詞多半是主觀的，取決於個人感受",
     "opts": [
      "氣味詞多半是主觀的，取決於個人感受",
      "英文完全沒有描述氣味的字",
      "氣味比顏色容易描述",
      "這些字只用在香水上"
     ]
    },
    {
     "q": "研究顯示，英國人最少談論哪一類感官經驗？",
     "a": "嗅覺（聞到的東西）",
     "opts": [
      "嗅覺（聞到的東西）",
      "視覺（看到的東西）",
      "聽覺（聽到的東西）",
      "味覺（吃到的東西）"
     ]
    }
   ]
  }
 ]
});
